# Publishing FP.Arrow.Core to NuGet

This repository publishes **FP.Arrow.Core** using [NuGet trusted publishing](https://learn.microsoft.com/en-us/nuget/nuget-org/trusted-publishing) (OIDC). No long-lived NuGet API keys are stored in GitHub.
...
1. Runs tests
2. Packs `FP.Arrow.Core` with the tag version
...
- **Package ID taken** — Change `PackageId` in `Arrow.Core.csproj` if `FP.Arrow.Core` is unavailable on nuget.org.
- **Symbols 404: "package does not exist"** — Symbols require the main `.nupkg` to be **validated and live** on nuget.org first. The workflow waits up to 20 minutes after the main push before uploading symbols. If the job fails at the wait step, the main package did not publish: check **Manage Packages** and your `NUGET_USERNAME` / trusted publisher setup, then publish a **new version**.
