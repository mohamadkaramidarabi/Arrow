...
- **Symbols 404: "package does not exist"** — NuGet requires the main `.nupkg` to be published before `.snupkg`. Do not re-run only the symbol step; re-run the full **Publish NuGet** workflow (or push the main package first). The workflow uses explicit filenames and retries symbol upload after indexing.
